(() => {
  ...
  var $schedulesTable = document.querySelector("#schedulesTable"),
    $tableTemplate =
      '<li class="parent"><ul class="scheduleTable"><li class="title">{{name}}</li>{{content}}</ul>',
    $cellTemplate =
      '<li><div class="className">{{className}}</div><pre class="time">{{startTime}} — {{endTime}}</pre></li>';
  ...
  getData()
    .then((bsData) => {
      ...
      if (!bsData.schedules) {
        $schedulesTable.innerHTML =
          '<li><div class="className">No schedules were found.</div></li>';
        return;
      }
      ...
      var scheduleKeys = Object.keys(schedules)
          ...
          .filter((key) => {
            if (
              schedules[key].hidden ||
              schedules[key].name === "Empty Schedule"
            )
              return false;
            return true;
          }),
      ...
      scheduleKeys.forEach((scheduleKey) => {
        var schedule = schedules[scheduleKey],
          name = schedule.name.replace(/^![0-9]+ |^!/, ""),
        ...
        keys.forEach((key) => {
          var period = schedule[key],
            ...
            startTime = data.displayTime(
              data.dateFromTimeString(date, period.start),
            ),
            endTime = data.displayTime(
              data.dateFromTimeString(date, period.end),
            ),
    ...
    .catch((error) => {
      console.log(error);
      $schedulesTable.innerHTML =
        '<li><div class="className">Error.</div></li>';
    });
})();
