  ...
  var currAnimationStage = -1,
    ...
    startLoadingAnimation = () => {
      ...
      loadingAnimationID = setInterval(() => {
        currAnimationStage++;
        if (currAnimationStage >= loadingAnimationStages.length)
          currAnimationStage = 0;
  ...
  getData()
    .then((bsData) => {
      ...
      setInterval(() => {
        ...
        var date = new Date();
        $startTimeView.innerText = data.displayTime(
          data.dateFromTimeString(date, period.start),
        );
        $endTimeView.innerText = data.displayTime(
          data.dateFromTimeString(date, period.end),
        );
        var secsLeft = Math.floor(
          (data.dateFromTimeString(date, period.end).getTime() -
            date.getTime()) /
            1000,
        );
        $countdownView.innerText =
